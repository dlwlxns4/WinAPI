...
private:
	...
	__int64 periodFrequency;		// 고성능 타이머의 주파수( 초당 진동 수 ), 지원하지 않으면 0
	float timeScale;				// 1초를 초당 진동 수로 나눠서 경과 진동 수를 시간 단위로 변환
	__int64 currTime;				// 현재 진동 누적 수 (현재 시간)
	__int64 lastTime;				// 이전 진동 누적 수 (이전 시간)
