...
void TilemapToolScene::Update()
{
    ...
    if (PtInRect(&(sampleArea), g_ptMouse))
    {
        if (KeyManager::GetSingleton()->IsOnceKeyDown(VK_LBUTTON))
        {
            ...
            cout << "from " << selectedIdY << " " << selectedIdX << endl;
        }
    }
    if (PtInRect(&(sampleArea), g_ptMouse))
    {
        ...
        {
            int posX = g_ptMouse.x - sampleArea.left;
            int selectedOutIdX = posX / TILE_SIZE;

            int posY = g_ptMouse.y - sampleArea.top;
            int selectedOutIdY = posY / TILE_SIZE;

            
            cout << "to " << selectedOutIdY << " " << selectedOutIdX << endl;
            for (int i = selectedIdY; i <= selectedOutIdY; i++)
            {
                for (int j = selectedIdX; j <= selectedOutIdX; j++)
                {  
                    cout << i << " " << j << endl;
                    SAMPLE_TILE_INFO sample;
                    sample.frameX = sampleTileInfo[i * SAMPLE_TILE_COUNT_X + j].frameX;
                    sample.frameY = sampleTileInfo[i * SAMPLE_TILE_COUNT_X + j].frameY;
                    selectedSampleTiles.push_back(sample);
                }
            }

            vertical = selectedOutIdX - selectedIdX;
            horizontal = selectedOutIdY - selectedIdY;
    ...
    for (int i = 0; i < TILE_COUNT_X * TILE_COUNT_Y; i++)
    {
        if (PtInRect(&(tileInfo[i].rc), g_ptMouse))
        {
            if (KeyManager::GetSingleton()->IsStayKeyDown(VK_LBUTTON))
            {
                ...
                {
                    for (int k = 0; k <= vertical; k++, index++)
                    {
                        if (i + k + j * TILE_COUNT_Y< TILE_COUNT_X* TILE_COUNT_Y)
                        {
                            if(i + k + j * TILE_COUNT_Y < (i + k + j * TILE_COUNT_Y /TILE_COUNT_Y)*(TILE_COUNT_X)+(TILE_COUNT_X))
                            {
                                tileInfo[i+ k + j * TILE_COUNT_Y].frameX = selectedSampleTiles[index].frameX;
                                tileInfo[i+ k + j * TILE_COUNT_Y].frameY = selectedSampleTiles[index].frameY;
                            }
                        }
...
void TilemapToolScene::Render(HDC hdc)
{
    ...
    */
    ...
    for (int i = 0; i < selectedSampleTiles.size(); i++)
    {
        sampleImage->Render(hdc, TILEMAPTOOL_SIZE_X - sampleImage->GetWidth()+selectedSampleTiles[i].frameX*TILE_SIZE + sampleImage->GetFrameWidth() / 2,
            sampleImage->GetHeight() + sampleImage->GetFrameHeight() / 2 + 180+selectedSampleTiles[i].frameY*TILE_SIZE,
            selectedSampleTiles[i].frameX, selectedSampleTiles[i].frameY);
    }
